...
private enum PartygramSettingsEntry: ItemListNodeEntry {
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        case let .attachmentsFolder(text, value):
            return ItemListDisclosureItem(presentationData: presentationData, systemStyle: .glass, title: text, label: value, labelStyle: .text, sectionId: self.section, style: .blocks, disclosure: .none, action: {
...
private func partygramSettingsEntries(settings: ExperimentalUISettings) -> [PartygramSettingsEntry] {
    ...
    entries.append(.useDelayInfo("Автоматически ставит задержку в ~12 секунд при отправке сообщений. При использовании это замедляет отправку."))
    ...
    entries.append(.saveReadDateInfo("Локально сохраняет данные о чтении сообщений. Будет использоваться, если Telegram не предоставил эту информацию."))
    entries.append(.saveLastOnline("Сохранять последний онлайн", settings.partygramSpySaveLastOnline))
    entries.append(.saveLastOnlineInfo("Сохраняет последний известный онлайн для людей со скрытым последним посещением. Вы можете видеть эту информацию в профиле."))
...
public func partygramSettingsController(context: AccountContext) -> ViewController {
    ...
    let updateSettings: @escaping ((@escaping (ExperimentalUISettings) -> ExperimentalUISettings) -> Void) = { f in
    ...
    |> map { presentationData, sharedData -> (ItemListControllerState, (ItemListNodeState, Any)) in
        ...
        let controllerState = ItemListControllerState(presentationData: ItemListPresentationData(presentationData), title: .text("Настройки Partygram"), leftNavigationButton: nil, rightNavigationButton: nil)
    ...
    let controller = ItemListController<PartygramSettingsEntry>(context: context, state: signal)
    presentControllerImpl = { [weak controller] c in
        controller?.present(c, in: .window(.root), with: ViewControllerPresentationArguments(sourceNodeRect: { _ in CGRect() }))
    }
    return controller
}
