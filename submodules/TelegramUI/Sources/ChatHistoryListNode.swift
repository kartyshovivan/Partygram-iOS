...
private func partygramFilterDeletedMessages(from entries: [ChatHistoryEntry], saveDeletedMessages: Bool) -> [ChatHistoryEntry] {
    if saveDeletedMessages {
        return entries
    }

    return entries.compactMap { entry -> ChatHistoryEntry? in
        switch entry {
        ...
        case let .MessageGroupEntry(groupInfo, messages, presentationData):
            let filteredMessages = messages.filter { item in
                return !item.0.attributes.contains(where: { $0 is PartygramDeletedMessageAttribute })
            }
            if filteredMessages.isEmpty {
                return nil
            }
            return .MessageGroupEntry(groupInfo, filteredMessages, presentationData)
        default:
            return entry
        }
    }
...
private func partygramShouldSkipReadMessages(_ settings: ExperimentalUISettings) -> Bool {
...
private func extractAssociatedData(
    ...
    showSecondsInMessageTimestamps: Bool
    ...
    private func beginChatHistoryTransitions(resetScrolling: Bool, switchedToAnotherSource: Bool) {
        ...
        let historyViewTransitionDisposable = (combineLatest(queue: messageViewQueue,
            ...
            case let .HistoryView(view, type, scrollPosition, flashIndicators, originalScrollPosition, data, id):
                ...
                let (rawFilteredEntries, updatedChatHistoryEntriesForViewState) = chatHistoryEntriesForView(
                ...
                let filteredEntries = partygramFilterDeletedMessages(from: rawFilteredEntries, saveDeletedMessages: context.sharedContext.immediateExperimentalUISettings.partygramSpySaveDeletedMessages)
        ...
        self.readHistoryDisposable.set((readHistory |> deliverOnMainQueue).startStrict(next: { [weak self] messageIndex, canRead in
            ...
            if apply {
                ...
                case .peer, .replyThread:
                    if !partygramShouldSkipReadMessages(strongSelf.context.sharedContext.immediateExperimentalUISettings) && !strongSelf.context.account.isSupportUser {
    ...
    private func processDisplayedItemRangeChanged(displayedRange: ListViewDisplayedItemRange, transactionState: ChatHistoryTransactionOpaqueState) {
        ...
        if let visible = displayedRange.visibleRange {
            ...
            if !messageIdsWithUnseenReactions.isEmpty {
                ...
                if self.canReadHistoryValue && !partygramShouldSkipReadMessages(self.context.sharedContext.immediateExperimentalUISettings) {
