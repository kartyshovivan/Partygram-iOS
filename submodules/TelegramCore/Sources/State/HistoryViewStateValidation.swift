                        ...
                        for id in removedMessageIds {
                            if !validMessageIds.contains(id) {
                                if let tag = tag {
                                    ...
                                    markPartygramMessageDeleted(transaction: transaction, id: id, timestamp: Int32(Date().timeIntervalSince1970))
                                    Logger.shared.log("HistoryValidation", "marking message \(id) in \(id.peerId) as deleted")
                                }
                            }
                        }
                    }
                ...
                return postbox.transaction { transaction -> Void in
                    ...
                    for id in removedMessageIds {
                        if !validMessageIds.contains(id) {
                            ...
                            Logger.shared.log("HistoryValidation", "marking thread message \(id) in \(id.peerId) as deleted")
                        }
                    }
                }
            }
