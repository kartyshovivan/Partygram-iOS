...
private func updatedPartygramEditHistoryAttributes(previousMessage: Message, updatedAttributes: [MessageAttribute]) -> [MessageAttribute] {
    ...
    var entries = (previousMessage.attributes.first(where: { $0 is PartygramEditHistoryMessageAttribute }) as? PartygramEditHistoryMessageAttribute)?.entries ?? []
    if entries.last?.text != previousMessage.text {
        var date = previousMessage.timestamp
        if let editedAttribute = previousMessage.attributes.first(where: { $0 is EditedMessageAttribute }) as? EditedMessageAttribute, editedAttribute.date != 0 {
            date = editedAttribute.date
        }
        entries.append(PartygramMessageEditHistoryEntry(date: date, text: previousMessage.text))
    }
    if entries.count > 30 {
        entries = Array(entries.suffix(30))
    }

    var result = updatedAttributes.filter { !($0 is PartygramEditHistoryMessageAttribute) }
    if !entries.isEmpty {
        result.append(PartygramEditHistoryMessageAttribute(entries: entries))
    }
    return result
}
        ...
        switch operation {
            ...
            case let .DeleteMessagesWithGlobalIds(ids):
                ...
                for id in transaction.messageIdsForGlobalIds(ids) {
                    markPartygramMessageDeleted(transaction: transaction, id: id, timestamp: timestamp)
                }
                ...
                for (peerId, status) in statuses {
                    if peerId == accountPeerId {
                        if explicit {
                            ...
                            var partygramRemoteOnlineUntilTimestamp: Int32?
                            switch status {
                                case let .userStatusOnline(userStatusOnlineData):
                                    ...
                                    partygramRemoteOnlineUntilTimestamp = timestamp
                            ...
                            } else if let partygramRemoteOnlineUntilTimestamp = partygramRemoteOnlineUntilTimestamp {
                                transaction.updatePreferencesEntry(key: PreferencesKeys.partygramAccountPresenceSettings, { current in
                                    let settings = current?.get(PartygramAccountPresenceSettings.self) ?? .defaultSettings
                                    return PreferencesEntry(settings.withUpdatedRemoteOnlineUntilTimestamp(partygramRemoteOnlineUntilTimestamp))
                                })
                            }
                        }
                    } else {
